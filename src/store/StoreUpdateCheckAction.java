...
public class StoreUpdateCheckAction extends Action {
	public String execute(
		...
		//店舗を更新する管理者のidを得るためにセッションの情報を管理者beanに詰める
		Admin admin = (Admin)session.getAttribute("admin");

		//店舗beanの各項目にパラメータをうつす
		String store_id = (String)((Store)session.getAttribute("store_update")).getStore_id();
		...
		if(session.getAttribute("filename_update") != null) {
			filename = (String)session.getAttribute("filename_update");
		} else {
			filename = (String)((Store)session.getAttribute("store_update")).getFilename();
		}
